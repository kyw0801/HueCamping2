	...
	%>
		...
		%>
			<tr class="lcategory">
			 ...
			 <td><a href="deleteLCategory.net?lno=<%=cbean.getLno()%>&lstep=<%=cbean.getLstep()%>">삭제</a></td>
			 <td><a href="javascript:insertSmallCategory(<%=cbean.getLno()%>)" id="showInsertSForm">+</a></td>
			 <td><a href="largeCategoryUp.net?lno=<%=cbean.getLno()%>&lstep=<%=cbean.getLstep()%>">▲</a> | <a href="largeCategoryDown.net?lno=<%=cbean.getLno()%>&lstep=<%=cbean.getLstep()%>">▼</a></td>
			</tr>
		...
		if(cbean.getSname() != null){%>
			<tr class="scategory">
			 ...
			 <td><a href="#" id="showForm">수정</a></td>
			 <td><a href="deleteSCategory.net?sno=<%=cbean.getSno()%>&sstep=<%=cbean.getSstep()%>">삭제</a></td>
			 <td></td>
			 <td><a href="smallCategoryUp.net?lno=<%=cbean.getLno()%>&sno=<%=cbean.getSno()%>&sstep=<%=cbean.getSstep()%>">△</a> | <a href="smallCategoryDown.net?lno=<%=cbean.getLno()%>&sno=<%=cbean.getSno()%>&sstep=<%=cbean.getSstep()%>">▽</a></td>
			</tr>
			<tr class="updateCateTd" style="display: none;">
			 <td colspan="5"> 
