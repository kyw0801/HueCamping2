...
public class CategoryDao {
	...
	public ArrayList<CategoryBean> getOnlyLargeCategory(){
		...
		try {
			// 3. SQL 작상 및 분석
			String sql = "select * from bigcategory order by step asc"; 
			ps = conn.prepareStatement(sql);
	...
	public ArrayList<CategoryBean> getOnlySmallCategory(int lno){
		...
		try {
			// 3. SQL 작상 및 분석
			String sql = "select * from smallcategory where lcno=? order by lcno asc, step asc"; 
			ps = conn.prepareStatement(sql);
	...
	public int countLcategory() { // lcategory 테이블의 모든 행의 개수 반환
		...
		try {
			// 3. SQL 작상 및 분석
			String sql = "select count(*) as cnt from bigcategory";
			ps = conn.prepareStatement(sql);
	...
	public int countScategory(int lno) { // scategory 테이블의 모든 행의 개수 반환
		...
		try {
			// 3. SQL 작상 및 분석
			String sql = "select count(*) as cnt from smallcategory where lcno=" + lno;
			ps = conn.prepareStatement(sql);
	...
	public int updateDownLStep(int lno, int lstep) { // 2  1
		...
		try {
			...
			String sql_1 = "update bigcategory set step = step-1 where step = ? + 1";
			...
			String sql_2 = "update bigcategory set step = step+1 where no=?";
	...
	public int updateDownSStep(int lno, int sno, int sstep) {
		...
		try {
			// 3. SQL 작상 및 분석
			String sql_1 = "update smallcategory set step = step-1 where step = ? + 1 and lcno=?";
			ps = conn.prepareStatement(sql_1);
			ps.setInt(1, sstep);
			ps.setInt(2, lno);
			...
			String sql_2 = "update smallcategory set step = step+1 where no=?";
			ps = conn.prepareStatement(sql_2);
			ps.setInt(1, sno);
	...
	public int insertLCategory(String lcname) {
		...
		try {
			// 3. SQL 작상 및 분석
			String sql_1 = "select max(step) from bigcategory";
			ps = conn.prepareStatement(sql_1);
			...
			String sql_2 = "insert into bigcategory values(bigcateseq.nextval, ?, ?)";
			ps = conn.prepareStatement(sql_2);
	...
	public int insertSCategory(int lno, String scname) {
		...
		try {
			// 3. SQL 작상 및 분석
			String sql_1 = "select max(step) from smallcategory where lcno=" + lno;
			ps = conn.prepareStatement(sql_1);
			...
			String sql_2 = "insert into smallcategory values(smallcateseq.nextval, ?, ?, ?)";
			ps = conn.prepareStatement(sql_2);
	...
	public int deleteSCategory(int sno, int sstep){
		...
		try {
			...
			// 3. SQL 작상 및 분석
			String sql_1 = "update smallcategory set step=step-1 where lcno=(select lcno from smallcategory where no=?) and step>?";
			ps = conn.prepareStatement(sql_1);
			ps.setInt(1, sno);
			ps.setInt(2, sstep);
			...
			// 3. SQL 작상 및 분석
			String sql_2 = "delete from smallcategory where no=" + sno;
			ps = conn.prepareStatement(sql_2);
	...
	public int deleteLCategory(int lno, int lstep){
		...
		try {
			...
			// 3. SQL 작상 및 분석
			String sql_1 = "update bigcategory set step=step-1 where step>?";
			ps = conn.prepareStatement(sql_1);
			ps.setInt(1, lstep);
			...
			// 3. SQL 작상 및 분석
			String sql_2 = "delete from bigcategory where no=" + lno;
			ps = conn.prepareStatement(sql_2);
	...
	public int updateLCategoryName(int lno, String lcname) {
		...
		try {
			...
			// 3. SQL 작상 및 분석
			String sql = "update bigcategory set name=? where no=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, lcname);
			ps.setInt(2, lno);
	...
	public int updateSCategoryName(int sno, String scname) {
		...
		try {
			...
			// 3. SQL 작상 및 분석
			String sql = "update smallcategory set name=? where no=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, scname);
			ps.setInt(2, sno);
